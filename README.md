...
`should_write_rag=true` 的内容才写入**任务发起用户的私有 RAG**。当前版本：
...
| 评测 | 当前证据 | 诚实边界 |
|---|---|---|
| Text2SQL | 固定 60 题执行结果等价回归集 60/60；首次 59/60，重试修复 1 题 | 固定集通过不等于未知问法 100% 泛化 |
| 意图路由 | 110 条五分类固定回归集 110/110；固定集全部命中确定性规则，0 次 LLM 调用，p50 0.05 ms、p95 0.16 ms | 这是常见路由防回归，不是线上泛化率；该集合未衡量新问题的 LLM 兜底准确率 |
